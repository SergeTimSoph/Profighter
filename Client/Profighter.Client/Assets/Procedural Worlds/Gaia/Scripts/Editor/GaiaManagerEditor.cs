        ...
        private void DoMaintenance(bool forceMaintenanceRun = false)
        {
            ...
            if (!m_settings.m_shaderReimportRestartRequired && ((!System.IO.Directory.Exists(GaiaUtils.GetAssetPath("Dev Utilities")) && File.Exists(GaiaDirectories.GetSettingsDirectory() + "\\" + GaiaConstants.maintenanceTokenFilename))) || forceMaintenanceRun)
            {
                ...
                else
                {
                    ...
                    {
                        if (EditorUtility.DisplayDialog("Gaia Installer", "Gaia is installing, please wait as this will take around 5 minutes.\r\n\r\nIn the meantime, please subscribe on our web site for access to release discounts, update notifications, tutorials and support.", "Yes, Subscribe now!", "No thanks"))
                        {
                            Application.OpenURL("https://www.procedural-worlds.com/subscribe?referrer=UnityEditor");
                        }
                    }
