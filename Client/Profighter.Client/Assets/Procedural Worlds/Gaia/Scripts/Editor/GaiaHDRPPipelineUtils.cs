    ...
    public static class GaiaHDRPPipelineUtils
    {
        ...
        private static void SetGlobalSettingsAsset(UnityPipelineProfile profile)
        {
            ...
            if (gaiaSettings != null)
            {
                ...
                if (!otherSettingsExist || EditorUtility.DisplayDialog("Setup HDRP Global Settings object?", "Do you want Gaia to set up an HDRP Global Settings Object for you? (Recommended).\r\n\r\nThis asset contains settings that ensure Gaia works correctly in HDRP. If you do not use these Global Settings, you would need to add the diffusion profiles that come with Gaia into the HDRP Global Settings manually afterwards.", "Add Global Settings", "Skip"))
                {
                    GraphicsSettings.RegisterRenderPipelineSettings<UnityEngine.Rendering.HighDefinition.HDRenderPipeline>(AssetDatabase.LoadAssetAtPath<RenderPipelineGlobalSettings>(GaiaUtils.GetAssetPath(globalSettingsAssetName + GaiaConstants.gaiaFileFormatAsset)));
                }
