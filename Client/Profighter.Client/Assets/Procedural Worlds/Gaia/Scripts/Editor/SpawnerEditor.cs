...
#region BiomePreset
        ...
        private void OnWorldMapStampingFinished()
        {
            ...
            finally
            {
                ...
                //Run the stitching process again - on steep terrains it can happen that corners are not stitched properly since the terrain might not have been present yet at that time.
                GaiaTerrainStitcher.m_extraSeamSize = 5;
                if (GaiaUtils.HasDynamicLoadedTerrains())
                {
                    GaiaUtils.CallFunctionOnDynamicLoadedTerrains(GaiaTerrainStitcher.StitchTerrain, false);
                }
                else
                {
